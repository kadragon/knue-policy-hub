...
- [ ] [debt] `headerDepth()`가 접지 못한 다단 헤더는 스팬 텍스트를 열마다 반복 출력 — 시설 사용료 종합운동장 표(`구 분` ×4)는 재파싱본이 기존 수작업 표보다 열등해 교체하지 않고 남겨 뒀다. 열마다 반복되는 증상은 본문 행과 밴드 전체를 덮은 헤더 셀에서 해소됐고, 남은 것은 접기 자체가 실패하는 케이스의 조건 완화 후 재대조 (source: task-next) — `tools/extract_body.js` `headerDepth()`
...
- [ ] [debt] 가로 스팬이 위에서 내려온 세로 스팬 슬롯을 덮어써 값이 사라진다 — `<td>A</td><td rowspan=2>B</td>` 다음 행에 `<td colspan=2>C</td>`가 오면 `B`가 유실된다. 전개 루프의 `dc` 구간에서 이미 점유된 슬롯을 건너뛸 것. 이번 변경 이전에도 있던 동작이지만 원점 출력이 붙은 뒤로는 의도된 빈 셀처럼 보인다 (source: code-review P3) — `tools/extract_body.js` `extractTable()`
