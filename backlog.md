...
- [ ] [debt] 가로 스팬 셀이 덮은 열마다 반복 출력된다 — 조직 설치 규정 헤더가 `조 직` ×5, 본문이 `교수지원과` ×4. 신규 표 행 992개 중 157개. 텍스트 동일성이 아니라 `src`/원점 열 정보로 판단해 원점에서 한 번만 찍고 나머지를 비울 것 (source: code-review P2) — `tools/parse_preview.py`
- [ ] [debt] `joinPieces()`는 단일 문자 런만 붙인다 — 다중 문자 래핑 라벨(`입학인재` + `관리과`, `부처` + `국본부장`)은 값 목록(`합격`/`불합격`)과 구분할 수 없어 구분자를 유지한다. 스팬/폭 정보 등 텍스트 밖 신호로 판정 가능한지 검토 (source: code-review P1/P2) — `tools/parse_preview.py:59`
- [ ] [debt] 공간관리 별표1 헤더 접힘이 열 이름을 오기 — `교양(공통)강의실 / 기본`, `… / 배정`. 원문은 `교양(공통)강의실`과 `기본 배정` 두 열. 헤더 밴드 접기에도 래핑 결합 규칙을 적용할 것 (source: code-review P2) — `tools/parse_preview.py:76`
