...
이슈당 한 번 호출. 툴 권한: Read + Write + Bash 금지(필요 없음). 모델: `haiku`.
...
셋 중 하나라도 실패하면 해당 이슈는 "failed — 서브에이전트 출력 품질 미달" 로 표시하고 파일을 `git restore` 로 되돌린 뒤 다음 이슈로 진행.

## 재실행 안전성

이미 처리된 이슈에 대해 `regulations.json` 의 `file_no` 가 이슈의 `새 fileNo` 와 같으면 스킵. 두 번 돌려도 중복 커밋이 생기지 않도록 한다.
